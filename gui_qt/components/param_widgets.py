...
from qtpy.QtWidgets import QLabel, QPushButton, QGridLayout, QTextEdit, QFileDialog


class InputParam(QtWidgets):
    ...
    def __init__(self, param, changed_callback=None, editing_callback=None, transient_for=None):
        ...
        self.label = QLabel(self.param.name)
        ...
        if self.param.dtype not in ignore_dtype_labels:
            self.dtype_label = QLabel(self.param.dtype)
            self.dtype_label.show()
            self.layout.addWidget(self.dtype_label, 0, 2)
...
class EntryParam(InputParam):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self._input.textChanged(self._mark_changed)
...
class FileParam(QtWidgets):
    """Provide an entry box for filename and a button to browse for a file."""

    def __init__(self, param, changed_callback=None, editing_callback=None, transient_for=None):
        self.param = param
        self.layout = QGridLayout()
        self.name_label = QLabel(self.param.name, self)
        self.layout.addWidget(self.name_label, 0, 0)

        self._open_button = QPushButton(text='...', parent=self)
        self._open_button.clicked.connect(self._handle_clicked)
        self.pack_start(self._open_button, False, False, 0)
        self.layout.addWidget(self._open_button, 0, 1)

        self.dtype_label = QLabel(self.param.dtype, self)
        self.layout.addWidget(self.dtype_label, 0, 2)
    ...
    def _handle_clicked(self, widget=None):
        ...
        if self.param.dtype == 'file_open':
            file_dialog = QFileDialog.getOpenFileName(self,
                tr("Open a Data File..."), action=Gtk.FileChooserAction.OPEN,
