...
class PropsDialog(QDialog):
    ...
    #                 param.hide = 'none'
    #             if param.category == cat and param.hide != "all":
    #                 param_name = QLabel(param.name)
    #                 qvb.addWidget(param_name, i, 0)
    #                 if param.dtype == "enum" or param.options:
    #                     dropdown = QComboBox()
    #                     for opt in param.options.values():
    #                         dropdown.addItem(opt)
    ...
    #                 else:
    #                     dtype_label = None
    #                 self._params_boxes.append((param_name, self.edit_params[-1], dtype_label))
    #             i += 1
    #         tab = QWidget()
    #         tab.setLayout(qvb)
    #         self.tabs.addTab(tab, cat)
    ...
    def update(self):
        ...
        """
        if self._block.__class__.__name__ != 'Options':
            return
        self.accept()
        ...
        #         children = current_widget.findChildren(QWidget, options=Qt.FindDirectChildrenOnly)

        #         i = 0
        #         selected_output_language = ''
        #         for param_gui in children:
        #             if param_gui.__class__ == QComboBox and self._block.__class__.__name__ == 'Options':
        #                 """
